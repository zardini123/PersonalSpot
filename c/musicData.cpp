...
// g++ -o musicData musicData.cpp -l sqlite3 -l boost_filesystem -l avformat -l avutil
...
int databaseSetup(const char* rootUrl) {
  ...
  fs::path baseUrlPath = rootUrl;
  baseUrlPath /= "Apollo";

  fs::path dbUrlPath = baseUrlPath / "apollo.db";
  baseUrl = strdup(baseUrlPath.c_str());
  ...
  if (!fs::exists(dbUrlPath)) {
    ret = fs::create_directories(baseUrlPath);
  }
...
int linkPersistent(const char* url) {
  ...
  fs::path album_art_path = fs::path(baseUrl) / "cache" / "album_art";

  fprintf(stderr, "dbUrl %s\n", dbUrl);

  //fprintf(stdout, "Preparing...\r");
  //fflush(stdout);
  ...
  //fprintf(stdout, "Final Link ID: %d\n", parentID);
  ...
  /* Get a count all files under directory for the progress bar */
  int fullCount;
  for (fs::recursive_directory_iterator iter(url), end; iter != end; ++iter) {
  ...
  for (fs::recursive_directory_iterator iter(url), end;
    ...
    if (fs::is_regular_file(iter->path())) {
      fflush(stdout);
      fprintf(stdout, "  \33[2K%d/%d", count, fullCount);
      fprintf(stdout, " \t%.2f%%\r", (count * 100)/((float)fullCount));
      //fprintf(stdout, " ...%s\r", path + strlen(path) - 20);
      ...
      if (prevHash != NULL) {
        ...
        if (art_index >= 0) {
          ...
          if (data_present) {
            ret = sqlite3_bind_int(ppSmtArtInsert, 3, input_frame->width);
            if (ret != SQLITE_OK) {
              fprintf(stderr, "Problem with bind: %s\n", sqlite3_errmsg(db));
              goto fileCleanup;
            }

            ret = sqlite3_bind_int(ppSmtArtInsert, 4, input_frame->height);
            if (ret != SQLITE_OK) {
              fprintf(stderr, "Problem with bind: %s\n", sqlite3_errmsg(db));
              goto fileCleanup;
            }
          ...
          if (prevHash != NULL) {
            ...
            strncpy(hashFront, res, 2);
            strncpy(hashBack, &res[2], 38);
            hashFront[2] = '\0';
            hashBack[38] = '\0';

            fs::path art_path = album_art_path / hashFront;

            if (!fs::exists(art_path)) {
              ret = fs::create_directories(art_path);
            }

            art_path /= hashBack;

            FILE *img = fopen(art_path.c_str(), "wb");
            ...
            //   goto fileCleanup;
            // }
            ret = sqlite3_bind_int(ppSmtArtInsert, 2, art_codec->id);
            if (ret != SQLITE_OK) {
              fprintf(stderr, "Problem with bind relativePath: %s\n", sqlite3_errmsg(db));
              goto fileCleanup;
            }
...
void errorCallback(void *avcl, int level, const char *fmt, va_list vl) {
  if (level <= AV_LOG_WARNING && level >= AV_LOG_PANIC)
    fprintf(stderr, "%d: %s", level, fmt);
}
